...
Tank::Tank(const std::shared_ptr<rclcpp::Node> nh, const std::string name, const QImage& tank_image, const QPointF& pos, float orient)
: nh_(nh)
, name_(name)
, tank_image_(tank_image)
, pos_(pos)
, orient_(orient)
, lin_vel_(0.0)
, ang_vel_(0.0)
...
, pen_(QColor(DEFAULT_PEN_R, DEFAULT_PEN_G, DEFAULT_PEN_B))
{
  ...
  velocity_sub_ = nh_->create_subscription<geometry_msgs::msg::Twist>((name_ + "/cmd_vel").c_str(), 10, [this](const geometry_msgs::msg::Twist::SharedPtr msg){
    last_command_time_ = std::chrono::system_clock::now();
...
//   teleport_relative_srv_ = nh_.advertiseService("teleport_relative", &Tank::teleportRelativeCallback, this);
//   teleport_absolute_srv_ = nh_.advertiseService("teleport_absolute", &Tank::teleportAbsoluteCallback, this);

  meter_ = tank_image_.height();
  rotateImage();
}


// void Tank::velocityCallback(const geometry_msgs::msg::Twist::SharedPtr vel)
// {
...
// bool Tank::setPenCallback(tanksim::SetPen::Request& req, tanksim::SetPen::Response&)
// {
//   pen_on_ = !req.off;
//   if (req.off)
//   {
...
// bool Tank::teleportRelativeCallback(tanksim::TeleportRelative::Request& req, tanksim::TeleportRelative::Response&)
// {
//   teleport_requests_.push_back(TeleportRequest(0, 0, req.angular, req.linear, true));
//   return true;
// }

// bool Tank::teleportAbsoluteCallback(tanksim::TeleportAbsolute::Request& req, tanksim::TeleportAbsolute::Response&)
// {
//   teleport_requests_.push_back(TeleportRequest(req.x, req.y, req.theta, 0, false));
//   return true;
// }

void Tank::rotateImage()
{
  QTransform transform;
  transform.rotate(-orient_ * 180.0 / PI + 90.0);
  tank_rotated_image_ = tank_image_.transformed(transform);
}

bool Tank::update(double dt, QPainter& path_painter, const QImage& path_image, qreal canvas_width, qreal canvas_height)
{
  bool modified = false;
  ...
  // Figure out (and publish) the color underneath the tank
  {
    tanksim::msg::Color color;
...
void Tank::paint(QPainter& painter)
{
  ...
  p.rx() -= 0.5 * tank_rotated_image_.width();
  p.ry() -= 0.5 * tank_rotated_image_.height();
  painter.drawImage(p, tank_rotated_image_);
}
